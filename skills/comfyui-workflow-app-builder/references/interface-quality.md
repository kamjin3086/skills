...
- Loading: choose wait treatments that fit the workflow duration. For quick image workflows, reassure the user with short copy such as "almost ready; keep this page open" and avoid overexplaining. For slow video workflows, set expectations with concrete duration copy, for example that a 10-second video may take 5-12 minutes, and ask the user to keep the page open. Prefer gentle ambient waiting animations or calm status cards over conventional percentage progress bars when progress is approximate, because precise bars can make long waits feel more stressful. Waiting copy must sound active and confidence-building; avoid words that imply the backend is lazy, slow by choice, idle, relaxed, or low-priority. Prefer language such as "rendering at full capacity", "processing continuously", or localized equivalents.
